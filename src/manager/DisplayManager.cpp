...
void DisplayManager_::loadNativeApps()
{
    ...
    // updateApp("webhook", Webhook, SHOW_WEBHOOK, 6);
...
void DisplayManager_::tick()
{
    ...
    else
    {
        ...
        else if (ui->getUiState()->appState == FIXED && appIsSwitching)
        {
            ...
            DEBUG_PRINTF("tick - CURRENT_APP: %s, TIME_PER_APP: %i", CURRENT_APP, TIME_PER_APP);
...
String DisplayManager_::getSettings()
{
    ...
    doc["WEBHOOK_PARAM"] = WEBHOOK_PARAM;
...
void DisplayManager_::setNewSettings(const char *json)
{
    ...
    WEBHOOK_URL = doc.containsKey("WEBHOOK_URL") ? doc["WEBHOOK_URL"].as<String>() : WEBHOOK_URL;
    WEBHOOK_PARAM = doc.containsKey("WEBHOOK_PARAM") ? doc["WEBHOOK_PARAM"].as<String>() : WEBHOOK_PARAM;
